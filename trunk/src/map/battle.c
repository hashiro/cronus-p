...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, int skill_num, int skill_lv, int wflag)
{
	...
	{	//Hit/Flee calculation
		...
		//TODO: iRO Wiki diz que não há mínimo e máximo de chance de esquiva na Guerra do Emperium, checar isso [Vianna]
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		{	//Calc base damage according to skill
			case NJ_ISSEN:
					wd.damage = sstatus->hp*80/100 + (sstatus->batk*40);//(HP*80%)+(Ataque*40)						
					if(sc && sc->data[SC_BUNSINJYUTSU] && sc->data[SC_BUNSINJYUTSU]->val2 > 0)
						ATK_ADDRATE(sc->data[SC_BUNSINJYUTSU]->val2*25);//(Imagem falsa = 25%+ de dano cada hit)
			...
			case ML_SPIRALPIERCE:
				...
				ATK_ADDRATE(200*skill_lv); //Skill modifier applies to weight only.
				i = sstatus->str/10;
				i*=i;
		...
		else
		{
			switch( skill_num )
			{
				...
				case RK_HUNDREDSPEAR: // Sugested formula from irowiki.
					skillratio += 500 + 60 * skill_lv; // Base skillratio.
					if( s_level > 100 ) skillratio += skillratio * (s_level - 100) / 200;	// Base level bonus.
					break;
				...
				case NC_AXEBOOMERANG:
					...
					break;
				case NC_POWERSWING:
					skillratio += 100 + 20 * skill_lv + sstatus->str + sstatus->dex;
					if( s_level > 100 ) skillratio += skillratio * (s_level - 100) / 200;	// Base level bonus.
					break;
				case NC_AXETORNADO:
...
// Cronus Changes
	{ "renewal_amotion_nerf",               &battle_config.renewal_amotion_nerf,            40,     0,         2000,        },
	{ "shield_penalty_rate",                &battle_config.shield_penalty_rate,				100,     0,      INT_MAX,        },
// EXP e Drop Nerf

	{ "renewal_drop_nerf",              &battle_config.renewal_drop_nerf,                  0,     0,            1,        },
	{ "renewal_exp_nerf",              &battle_config.renewal_exp_nerf,                  0,     0,            1,        },
	{ "renewal_party_exp_nerf",             &battle_config.renewal_party_exp_nerf,                  0,     0,            1,        },
};
