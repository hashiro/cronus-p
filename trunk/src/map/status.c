...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	...
	// nerf [Vianna]
	if ((amotion+=battle_config.renewal_amotion_nerf) > 2000)
		amotion = 2000;
...
static unsigned short status_base_atk(const struct block_list *bl, const struct status_data *status)
{
	...
	// [Skotlex]
	// TODO: Confirmar se a exclusividade dos players para essa fÛrmula
	// continua. [Vianna]
	if (bl->type == BL_PC)
		str+= ((TBL_PC*)bl)->status.base_level/4 + dex/5 + status->luk/3;
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	status->hit += 175 + level + status->dex + status->luk/3;
	status->flee += 100 + level + status->agi + status->luk/7;
	status->def2 += ((status->vit + level) >> 1) + (status->agi / 5);
	status->mdef2 += (status->int_ + (status->vit / 5) + (status->dex / 5) + (level >> 2));
...
static unsigned int status_base_pc_maxhp(struct map_session_data* sd, struct status_data* status)
{
	...
	/* Fix definitivo do HP - pequena variacao de 1 ponto pra mais ou pra menos corrigida.
	   A fÛrmula correta È MAX_HP = floor(floor(MAX_HP * TRANS_MODE) * (1 + VIT * 0.01)) ~ [ItachiSan] */
	...
	val = (int)((int)(val * classModifier) * (1 + status->vit * 0.01));
...
static unsigned int status_base_pc_maxsp(struct map_session_data* sd, struct status_data *status)
{
	...
	/* Fix definitivo do SP - pequena variacao de 1 ponto pra mais ou pra menos corrigida.
	   A fÛrmula correta È MAX_SP = floor(floor(MAX_SP * TRANS_MODE) * (1 + INT * 0.01)) ~ [ItachiSan] */

	val =  (int)((int)(val * classModifier) * (1 + status->int_ * 0.01));
...
{// Job-specific values (weight, HP, SP, ASPD)
	...
	{
		aspd_base[idx][i] = atoi(fields[i+10]);
	}
	return true;
}
...
int status_readdb(void)
{
	...
	memset(sp_coefficient2, 0, sizeof(sp_coefficient2));
