...
{
	uint8 buf[2];
	ShowInfo("Conexao de IP em progresso...\n");
	WBUFW(buf,0) = 0x2735;
...
int login_lan_config_read(const char *lancfgName)
{
	...
	if((fp = fopen(lancfgName, "r")) == NULL) {
		ShowWarning("Arquivo de configuracao de LAN nao encontrado: %s\n", lancfgName);
		return 1;
	}
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(sscanf(line,"%[^:]: %[^:]:%[^:]:%[^\r\n]", w1, w2, w3, w4) != 4)
		{
			ShowWarning("Erro na sintaxe do arquivo de configuracao %s na linha %d.\n", lancfgName, line_num);
			continue;
		}
		...
		if( strcmpi(w1, "subnet") == 0 )
		{
			...
			{
				ShowError("%s: Erro de configuracao: O char server (%s) e o map server (%s) pertencem a diferentes subnets!\n", lancfgName, w3, w4);
				continue;
			}
	...
	ShowStatus("Carregadas informacoes sobre %d subnetworks.\n", subnet_count);
...
int parse_console(const char* command)
{
	...
	{// commands with parameters
		...
		if( strcmpi(cmd, "create") == 0 )
		{
			...
			{
				ShowError("Console: Falha na criacao de contas.\n");
				return 0;
			}
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			{// authentication not found
				ShowStatus("Char-server '%s': Autenticacao requisitada pela conta %d RECUSADA (ip: %s).\n", server[id].name, account_id, ip);
				WFIFOHEAD(fd,25);
				WFIFOW(fd,0) = 0x2713;
				WFIFOL(fd,2) = account_id;
				WFIFOL(fd,6) = login_id1;
				...
				return 0;
		{
			...
			if( e_mail_check(email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de criar uma conta com email padrao foi RECUSADA - e-mail  (Conta: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( !accounts->load_num(accounts, &acc, account_id) || strcmp(acc.email, "a@a.com") == 0 || acc.email[0] == '\0' )
				ShowNotice("Char-server '%s': Tentativa de criar uma conta com email padrao foi RECUSADA - A conta nao existe ou o email nao e o padrao (account: %d, ip: %s).\n", server[id].name, account_id, ip);
			else {
				memcpy(acc.email, email, 40);
				ShowNotice("Char-server '%s': Criada conta com email padrao (Conta: %d, new e-mail: %s, ip: %s).\n", server[id].name, account_id, email, ip);
				// Save
				...
				return 0;
		{
			...
			if( e_mail_check(actual_email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de modificar email de uma conta (@email GM command), email invalido (account: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( e_mail_check(new_email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de modificar email de uma conta (@email GM command) email invalido (account: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( strcmpi(new_email, "a@a.com") == 0 )
				ShowNotice("Char-server '%s': Tentativa de modificar email de uma conta (@email GM command) com email padrao (account: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Tentativa de modificar email de uma conta (@email GM command), a conta nao existe (account: %d, ip: %s).\n", server[id].name, account_id, ip);
			else
			if( strcmpi(acc.email, actual_email) != 0 )
				ShowNotice("Char-server '%s': Tentativa de modificar email de uma conta (@email GM command), email incorreto (account: %d (%s), actual e-mail: %s, proposed e-mail: %s, ip: %s).\n", server[id].name, account_id, acc.userid, acc.email, actual_email, ip);
			else {
				safestrncpy(acc.email, new_email, 40);
				ShowNotice("Char-server '%s': Modificado email (@email GM command) (Conta: %d (%s), novo e-mail: %s, ip: %s).\n", server[id].name, account_id, acc.userid, new_email, ip);
				// Save
				accounts->save(accounts, &acc);
			}
		}
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro no status-change (Conta: %d nao encontrado, status sugerido %d, ip: %s).\n", server[id].name, account_id, state, ip);
			else
			if( acc.state == state )
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro na requisicao de banimento (Conta: %d nao encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				time_t timestamp;
				struct tm *tmtime;
				...
				if (timestamp == -1)
					ShowNotice("Char-server '%s': Erro na requisicao de banimento (Conta: %d, data invalida, ip: %s).\n", server[id].name, account_id, ip);
				else
				if( timestamp <= time(NULL) || timestamp == 0 )
					ShowNotice("Char-server '%s': Erro na requisicao de banimento (Conta: %d, a nova data desbane a conta, ip: %s).\n", server[id].name, account_id, ip);
				else
				{
					uint8 buf[11];
					char tmpstr[24];
					timestamp2string(tmpstr, sizeof(tmpstr), timestamp, login_config.date_format);
					ShowNotice("Char-server '%s': Requisicao de banimento (Conta: %d, O banimento termina em: %d (%s), ip: %s).\n", server[id].name, account_id, timestamp, tmpstr, ip);
				...
				return 0;
		{
			...
			if( acc.sex == 'S' )
				ShowNotice("Char-server '%s': Erro na mudanca de sexo - A conta selecionada e uma conta de sistema (Conta: %d, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				unsigned char buf[7];
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowStatus("Char-server '%s': recebendo (do char-server) o account_reg2 (Conta: %d nao encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				int len;
				int p;
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro na requisicao de desbanimento (Conta: %d nao encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			if( acc.unban_time == 0 )
				ShowNotice("Char-server '%s': Erro na requisicao de desbanimento (Conta: %d, nenhuma mudanca na data do banimento, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				ShowNotice("Char-server '%s': Requisicao de desbanimento (Conta: %d, ip: %s).\n", server[id].name, account_id, ip);
				acc.unban_time = 0;
				accounts->save(accounts, &acc);
			}
		}
		...
		case 0x2736: // WAN IP update from char-server
			...
			ShowInfo("Atualizado IP do servidor #%d to %d.%d.%d.%d.\n",id, CONVIP(server[id].ip));
			RFIFOSKIP(fd,6);
		break;

		case 0x2737: //Request to set all offline.
			ShowInfo("Desconectando usuarios %d.\n", id);
			online_db->foreach(online_db, online_db_setoffline, id);
			RFIFOSKIP(fd,2);
		break;

		default:
			ShowError("parse_fromchar: Packet desconhecido 0x%x vindo do char-server! Desconectando!\n", command);
			set_eof(fd);
			return 0;
		} // switch
	} // while
...
int mmo_auth_new(const char* userid, const char* pass, const char sex, const char* last_ip)
{
	...
	{
		ShowNotice("Registro de conta NEGADO (Limite de registros esgotado)\n");
		return 3;
	}
	...
	{
		ShowNotice("Tentativa de criar uma conta ja existente (Conta: %s_%c, pass: %s, senha recebida: %s)\n", userid, sex, acc.pass, pass);
	...
	ShowNotice("Criacao de conta (Conta %s, id: %d, pass: %s, sexo: %c)\n", acc.userid, acc.account_id, acc.pass, acc.sex);
...
int mmo_auth(struct login_session_data* sd)
{
	...
	{
		ShowNotice("Conexao rejeitada (Conta: %s, senha: %s, ID expirou, ip: %s)\n", sd->userid, sd->passwd, ip);
	...
	if( acc.unban_time != 0 && acc.unban_time > time(NULL) )
	{
		...
		ShowNotice("Conexao rejeitada (Conta: %s, Senha: %s, banida ate %s, ip: %s)\n", sd->userid, sd->passwd, tmpstr, ip);
		return 6; // 6 = Your are Prohibited to log in until %s
	}

	if( acc.state != 0 )
	{
		ShowNotice("Conexao rejeitada (Conta: %s, senha: %s, estado: %d, ip: %s)\n", sd->userid, sd->passwd, acc.state, ip);
		return acc.state - 1;
	}
	...
	if( sd->sex != 'S' && sd->account_id < START_ACCOUNT_NUM )
		ShowWarning("A conta %s possui o ID %d! Ids das contas devem ser superiores a %d para funcionarem corretamente!\n", sd->userid, sd->account_id, START_ACCOUNT_NUM);
...
void login_auth_ok(struct login_session_data* sd)
{
	...
	{
		ShowStatus("Conexao rejeitada: o nivel minimo de GM para se conectar e %d (Conta: %s, GM level: %d).\n", login_config.min_level_to_connect, sd->userid, sd->level);
		WFIFOHEAD(fd,3);
		WFIFOW(fd,0) = 0x81;
	...
	{// if no char-server, don't send void list of servers, just disconnect the player with proper message
		ShowStatus("Conexao recusada: Char-server nao esta online (Conta: %s).\n", sd->userid);
		WFIFOHEAD(fd,3);
		WFIFOW(fd,0) = 0x81;
		WFIFOB(fd,2) = 1; // 01 = Server closed
		WFIFOSET(fd,3);
...

	{
		...
		{// account is already marked as online!
			...
			{// Request char servers to kick this account out. [Skotlex]
				uint8 buf[6];
				ShowNotice("Usuario '%s' ja esta conectado - Conexao rejeitada.\n", sd->userid);
				WBUFW(buf,0) = 0x2734;
	...
	if( sd->level > 0 )
		ShowStatus("Conexao do GM (level:%d) '%s' aceita.\n", sd->level, sd->userid);
	else
...
void login_auth_failed(struct login_session_data* sd, int result)
{
	...
	if (login_config.log_login)
	{
		...
		case   3: error = "Recusado pelo servidor."; break; // 3 = Rejected from Server
		case   4: error = "Bloqueado pelo GM."; break; // 4 = You have been blocked by the GM Team
		case   5: error = "Cliente desatualizado."; break; // 5 = Your Game's EXE file is not the latest version
		...
		case   8: error = "Conta limitada"; break; // 8 = No more accounts may be connected from this company
		case   9: error = "Banido por DBA"; break; // 9 = MSI_REFUSE_BAN_BY_DBA
		...
		case  12: error = "Conexao recusada"; break; // 12 = MSI_REFUSE_TEMP_BAN_FOR_DBWORK
		case  13: error = "Conta bloqueada a pedido do usuario"; break; // 13 = MSI_REFUSE_SELF_LOCK
		case  14: error = "Servidor fechado"; break; // 14 = MSI_REFUSE_NOT_PERMITTED_GROUP
		case  15: error = "Servidor fechado"; break; // 15 = MSI_REFUSE_NOT_PERMITTED_GROUP
		case  99: error = "Conta apagada"; break; // 99 = This ID has been totally erased
		case 100: error = "Login invalido."; break; // 100 = Login information remains at %s
		case 101: error = "Em analise pela equipe."; break; // 101 = Account has been locked for a hacking investigation. Please contact the GM Team for more information
		case 102: error = "Em analise pela equipe."; break; // 102 = This account has been temporarily prohibited from login due to a bug-related investigation
		case 103: error = "Personagem sendo apagado."; break; // 103 = This character is being deleted. Login is temporarily unavailable for the time being
		case 104: error = "Personagem sendo apagado."; break; // 104 = This character is being deleted. Login is temporarily unavailable for the time being
...
int parse_login(int fd)
{
	...
	if( session[fd]->flag.eof )
	{
		ShowInfo("Conexao fechada '"CL_WHITE"%s"CL_RESET"'.\n", ip);
		do_close(fd);
		return 0;
	}
	...
	if( sd == NULL )
	{
		...
		{
			ShowStatus("Conexao recusada: O IP nao esta autorizado a conectar-se (recusado/aceito, ip: %s).\n", ip);
	...
	while( RFIFOREST(fd) >= 2 )
	{
		...
		}
		{
			...
			if( israwpass )
			{
				ShowStatus("Conexão requisitada por %s (ip: %s).\n", sd->userid, ip);
				...
				return 0;
		{
			...
			ShowInfo("Conexao requisitada pelo char-server '%s' @ %u.%u.%u.%u:%u (account: '%s', pass: '%s', ip: '%s')\n", server_name, CONVIP(server_ip), server_port, sd->userid, sd->passwd, ip);
		...
		default:
			ShowNotice("Conexao terminou de forma inesperada(ip: %s): Packet desconhecido 0x%x\n", ip, command);
			set_eof(fd);
			return 0;
		}
	}
...
int login_config_read(const char* cfgName)
{
	...
	ShowInfo("Carregando arquivos de configuracao %s...\n", cfgName);
	while(fgets(line, sizeof(line), fp))
	{
		if (line[0] == '/' && line[1] == '/')
			continue;
		...
		if(!strcmpi(w1,"timestamp_format"))
			...
			ShowStatus("Porta do login-server : %s\n",w2);
		}
	...
	fclose(fp);
	ShowInfo("Finalizada leitura do arquivo %s.\n", cfgName);
	return 0;
}
...
int do_init(int argc, char** argv)
{
	...
	if( accounts == NULL )
	{
		ShowFatalError("do_init: mecanismo da conta '%s' nao encontrado.\n", login_config.account_engine);
		exit(EXIT_FAILURE);
	}
	else
	{
		ShowInfo("Usando mecanismo de conta '%s'.\n", login_config.account_engine);
	...
	ShowStatus("O login-server esta "CL_GREEN"pronto"CL_RESET" (Servidor conectado na porta %u).\n\n", login_config.login_port);
